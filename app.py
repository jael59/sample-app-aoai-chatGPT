    ...
    try:
        ...
        if app_settings.azure_openai.function_call_azure_functions_enabled:
            ...
            if response.status_code == httpx.codes.OK:
                ...
                logging.error(f"Error getting tools metadata: {response.status_code}")
...
# prepare_model_args, promptflow_request, process_function_call,
# send_chat_request, complete_chat_request, AzureOpenaiFunctionCallStreamState,
# process_function_call_stream, stream_chat_request — unchanged from your original file.
...
async def conversation_internal(request_body, request_headers):
    try:
        ...
        if "messages" in request_body and request_body["messages"]:
            last_message = request_body["messages"][-1]["content"].strip()

        if last_message.lower().startswith("#note"):
            note_content = last_message.replace("#note", "").strip()
            await cosmos_db_ready.wait()
            if current_app.cosmos_conversation_client:
                await current_app.cosmos_conversation_client.container.create_item({
                    "id": str(uuid.uuid4()),
                    "type": "custom_knowledge",
                    "userId": "default",
                    "content": note_content
                })
            return jsonify({"answer": "Information noted."})

        stored_knowledge = ""
        if current_app.cosmos_conversation_client:
            query = "SELECT * FROM c WHERE c.type = 'custom_knowledge'"
            items_iter = current_app.cosmos_conversation_client.container.query_items(
                query=query,
                enable_cross_partition_query=True
            )
            matches = []
            async for item in items_iter:
                if item.get("content") and any(word in last_message.lower() for word in item["content"].lower().split()):
        ...
        if stored_knowledge:
            request_body["messages"].insert(0, {
                "role": "system",
                "content": f"Stored knowledge:\n{stored_knowledge}\nIf this answers the question, use it directly."
            })

        # --- End custom note handling ---
...
# --- New route to list all notes ---
@bp.route("/notes/list", methods=["GET"])
async def list_notes():
    await cosmos_db_ready.wait()
    if not current_app.cosmos_conversation_client:
        return jsonify({"error": "CosmosDB not configured"}), 500
    query = "SELECT * FROM c WHERE c.type = 'custom_knowledge'"
    items_iter = current_app.cosmos_conversation_client.container.query_items(
        query=query,
        enable_cross_partition_query=True
...
# --- All your /history/... endpoints from original file ---
# (They remain exactly the same; I haven’t removed anything.)

# generate_title function — unchanged from original
